...
//TODO: YOU NEED TO SYNC THIS FROM internal/generate/servicepackage/main.go.
// NOTE: here, I added parsing for ArnFormat.
			...
			case "ArnFormat":
				if attr, ok := args.Keyword["global"]; ok {
					...
					if len(args.Positional) == 1 {
						...
						v.errs = append(v.errs, fmt.Errorf("got multiple ArnFormat strings: %v", args.Positional))
					}
				}
	...
	for _, line := range funcDecl.Doc.List {
		...
		if m := annotation.FindStringSubmatch(line); len(m) > 0 {
			...
			case "ArnFormat", "IdAttrFormat", "NoImport", "Testing":
				// Ignored.
			default:
				fmt.Printf("[warn] decorator_resolver: unknown annotation: %s", annotationName)
			}
		}
	}
